...
(function(){
	...
	let playArea = null;
	let areaStyle = null;
	let direction = 'down'; // can be 'down', 'up', 'left', 'right'
	let id;
	...
	class Snake {
		...
		isFoodEaten() {
			let start = this.snakeParts[0];
            if ((parseInt(start.compStyle.top) == parseInt(food.compStyle.top)) && (parseInt(start.compStyle.left) == parseInt(food.compStyle.left))) {
                return true;
            }
			return false;
		}
		...
		initSnake() {
			...
			for (i = 1; i < this.snakeLength; i++) {
				const snakePart = this.snakeParts[0].part.cloneNode();
				snakePart.id = this.snakeParts[0].part.id + i;
				playArea.appendChild(snakePart);
				const newPart = new SnakePart(snakePart);
			...
			const foodPart = this.snakeParts[0].part.cloneNode();
			...
			const foodHeight = parseInt(food.compStyle.height) + SNAKE_SPACING;
			const foodWidth = parseInt(food.compStyle.width) + SNAKE_SPACING;
		...
		redrawSnake() {
			...
			let newTop = parseInt(this.snakeParts[0].compStyle.top);
			let newLeft = parseInt(this.snakeParts[0].compStyle.left);
	...
	document.addEventListener("DOMContentLoaded", function() {
		const snake = new Snake(SNAKE_LENGTH);
		snake.initSnake();
